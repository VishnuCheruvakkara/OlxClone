...
const PostAdForm = () => {
    ...
    const validateForm = () => {
        const newErrors = {};
        const titleRegex = /^[A-Za-z0-9][A-Za-z0-9\s]*$/;
        if (!productDetails.title) {
            newErrors.title = 'Title is required';
        } else if (!titleRegex.test(productDetails.title)) {
            newErrors.title = 'Title can only contain letters, numbers, and spaces, and cannot start with a space or contain only spaces';
        }
        ...
        // If errors are present, show a toast message and return falseFF
    ...
    const handleSubmit = async (event) => {
        ...
        if (!user) {
            toast.info("Please login to post a product!")
            return;
        }
        ...
        try {
            ...
            toast.error("Error found while add product!")
        }
        finally {
            setIsLoading(false); // Hide the spinner after submission is complete
        }
    };
    ...
    return (
        <div className="bg-gray-50 min-h-screen relative">
            ...
            {/* Conditionally render spinner based on isLoading */}
            {isLoading && (
